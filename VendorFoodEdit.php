...
// Load existing item (only allow editing your own vendor items)
$sqlFetch = "SELECT FoodID, FoodName, Price, Description, Category, DietaryTag, Status, Image
             FROM MENU_FOOD WHERE FoodID = ? AND VendorID = ? LIMIT 1";
$fetchStmt = mysqli_prepare($conn, $sqlFetch);
mysqli_stmt_bind_param($fetchStmt, "ii", $foodId, $vendorId);
mysqli_stmt_execute($fetchStmt);
$item = mysqli_fetch_assoc(mysqli_stmt_get_result($fetchStmt));
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['Action_Update'])) {
    ...
    $sqlUpdate = "UPDATE MENU_FOOD
                  SET FoodName = ?, Price = ?, Description = ?, Category = ?, DietaryTag = ?, Status = ?, Image = ?
                  WHERE FoodID = ? AND VendorID = ?";
