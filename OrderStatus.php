...
if ($orderId <= 0) {
    $latestSql = "SELECT OrderID FROM orders WHERE UserID = ? ORDER BY OrderID DESC LIMIT 1";
    ...
    if ($latestRow = mysqli_fetch_assoc($latestRes)) {
        $orderId = (int)$latestRow['OrderID'];
    }
}

$order = null;
if ($orderId > 0) {
    $orderSql = "SELECT o.OrderID, o.OrderType, o.PickupTime, o.TotalAmount, o.Status,
                        " . ($hasReservationModeColumn ? "o.ReservationMode" : "NULL AS ReservationMode") . ",
                        " . ($hasSeatCountColumn ? "o.SeatCount" : "NULL AS SeatCount") . ",
                        o.CancelReason,
...
// Logic to determine active progress step
$status = $order ? strtolower($order['Status']) : 'unknown';
$stepIndex = 1; // 1 = Pending, 2 = Preparing, 3 = Ready
$isCancelled = false;
...
if ($status === 'cancelled') {
    ...
    $stepIndex = 3;
} elseif (in_array($status, ['processing', 'preparing', 'accepted'])) {
    $stepIndex = 2;
} else {
    $stepIndex = 1; // pending/default
}
