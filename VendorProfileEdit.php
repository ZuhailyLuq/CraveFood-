...
$sql = "SELECT ShopName, OpenHours, Location, FoodType, Description, Image, Latitude, Longitude FROM vendor WHERE VendorID = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateSql = "UPDATE vendor SET ShopName = ?, OpenHours = ?, Location = ?, FoodType = ?, Description = ?, Image = ?, Latitude = ?, Longitude = ?, LastUpdate = NOW() WHERE VendorID = ?";
    $updateStmt = mysqli_prepare($conn, $updateSql);
    mysqli_stmt_bind_param($updateStmt, "ssssssssi", $shopName, $openHours, $location, $foodType, $description, $image, $latitude, $longitude, $vendorId);

    if (mysqli_stmt_execute($updateStmt)) {
        $_SESSION['ShopName'] = $shopName;

        // Mark all unread admin notifications as read after profile update
        $dismissStmt = mysqli_prepare($conn, "UPDATE admin_notifications SET IsRead = 1 WHERE VendorID = ? AND IsRead = 0");
        if ($dismissStmt) {
            mysqli_stmt_bind_param($dismissStmt, "i", $vendorId);
            mysqli_stmt_execute($dismissStmt);
        }
