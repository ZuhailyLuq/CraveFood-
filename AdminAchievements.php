...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        $achievementId = (int)($_POST['achievement_id'] ?? 0);
        if ($achievementId > 0) {
            $deleteClaims = mysqli_prepare($conn, "DELETE FROM user_achievement_claims WHERE AchievementID = ?");
            mysqli_stmt_bind_param($deleteClaims, "i", $achievementId);
            mysqli_stmt_execute($deleteClaims);

            $deleteAchievement = mysqli_prepare($conn, "DELETE FROM achievements WHERE AchievementID = ?");
            mysqli_stmt_bind_param($deleteAchievement, "i", $achievementId);
            if (mysqli_stmt_execute($deleteAchievement)) {
        ...
        $toggleStmt = mysqli_prepare($conn, "UPDATE achievements SET IsActive = ? WHERE AchievementID = ?");
        mysqli_stmt_bind_param($toggleStmt, "ii", $newStatus, $achievementId);
