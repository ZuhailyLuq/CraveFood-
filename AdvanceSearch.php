...
function menuColExists($conn, $col) {
    $c = mysqli_real_escape_string($conn, $col);
    $r = mysqli_query($conn, "SHOW COLUMNS FROM `MENU_FOOD` LIKE '$c'");
    return $r && mysqli_num_rows($r) > 0;
}
function vendorColExists($conn, $col) {
    $c = mysqli_real_escape_string($conn, $col);
    $r = mysqli_query($conn, "SHOW COLUMNS FROM `vendor` LIKE '$c'");
    return $r && mysqli_num_rows($r) > 0;
}

$hasRating  = menuColExists($conn, 'Rating');
$hasVendorId = menuColExists($conn, 'VendorID');
$hasLat     = vendorColExists($conn, 'Latitude');
$hasLng     = vendorColExists($conn, 'Longitude');
$hasLoc     = vendorColExists($conn, 'Location');
...
if ($keyword !== '') {
    ...
    $vendorConditions[] = "(mf.FoodName LIKE '%$kw%' OR mf.Description LIKE '%$kw%' OR v.ShopName LIKE '%$kw%')";
}
if ($dietary !== '') {
    $d = mysqli_real_escape_string($conn, $dietary);
    $vendorConditions[] = "mf.DietaryTag = '$d'";
}
if ($category !== '') {
    $c = mysqli_real_escape_string($conn, $category);
    $vendorConditions[] = "mf.Category = '$c'";
}
if ($minPrice !== '' && is_numeric($minPrice)) $vendorConditions[] = "mf.Price >= " . (float)$minPrice;
if ($maxPrice !== '' && is_numeric($maxPrice)) $vendorConditions[] = "mf.Price <= " . (float)$maxPrice;
if ($hasRating && $minRating !== '' && is_numeric($minRating)) $vendorConditions[] = "mf.Rating >= " . (float)$minRating;
...
if ($tableCheck && mysqli_num_rows($tableCheck) > 0 && $hasVendorId) {
    ...
    if ($userLat !== null && $userLng !== null && $hasLat && $hasLng) {
        ...
        $distanceHaving = "HAVING distance_km <= " . (float)$radiusKm;
    }
...
$aStmt = mysqli_prepare($conn, "SELECT OrderID, Status FROM orders WHERE UserID = ? AND Status NOT IN ('Finished','Completed','Cancelled') ORDER BY OrderID DESC LIMIT 1");
mysqli_stmt_bind_param($aStmt, "i", $uid);
mysqli_stmt_execute($aStmt);
$aRes = mysqli_stmt_get_result($aStmt);
if ($aRes) $activeOrder = mysqli_fetch_assoc($aRes);
