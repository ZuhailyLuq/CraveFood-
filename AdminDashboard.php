...
if ($row = mysqli_fetch_assoc($r)) $userCount = (int)$row['c'];

$r = mysqli_query($conn, "SELECT COUNT(*) AS c FROM vendor");
if ($row = mysqli_fetch_assoc($r)) $vendorCount = (int)$row['c'];

$r = mysqli_query($conn, "SELECT COUNT(*) AS c FROM orders");
if ($row = mysqli_fetch_assoc($r)) $orderCount = (int)$row['c'];

/* ── Real weekly trends ──────────────────────── */
// Users this week vs last week
function weekCount($conn, $table, $col, $weeksAgo) {
    $end   = date('Y-m-d', strtotime("-{$weeksAgo} week"));
    $start = date('Y-m-d', strtotime("-" . ($weeksAgo + 1) . " week"));
    $sql   = "SELECT COUNT(*) AS c FROM `$table` WHERE `$col` >= '$start' AND `$col` < '$end'";
...
if (colExists($conn, 'user', 'CreatedAt')) {
    $u1 = weekCount($conn, 'user', 'CreatedAt', 0); // this week
    $u2 = weekCount($conn, 'user', 'CreatedAt', 1); // last week
    $userTrend = ($u2 > 0) ? round((($u1 - $u2) / $u2) * 100) : ($u1 > 0 ? 100 : 0);
}

// orders: prefer CreatedAt, fall back to OrderDate
$orderDateCol = null;
if (colExists($conn, 'orders', 'CreatedAt'))  $orderDateCol = 'CreatedAt';
elseif (colExists($conn, 'orders', 'OrderDate')) $orderDateCol = 'OrderDate';

if ($orderDateCol) {
    $o1 = weekCount($conn, 'orders', $orderDateCol, 0);
    $o2 = weekCount($conn, 'orders', $orderDateCol, 1);
    $orderTrend = ($o2 > 0) ? round((($o1 - $o2) / $o2) * 100) : ($o1 > 0 ? 100 : 0);
}

/* ── Vendor update status ────────────────────── */
$thresholdDays  = 30;
$vendorsSql     = "SELECT VendorID, ShopName, LastUpdate FROM vendor ORDER BY LastUpdate ASC";
...
function trendChip($pct) {
    if ($pct === null) {
        return '<div class="stat-trend neutral">— no trend data</div>';
    }
    if ($pct > 0) {
        return '<div class="stat-trend up">▲ +' . $pct . '% this week</div>';
    } elseif ($pct < 0) {
        return '<div class="stat-trend down">▼ ' . $pct . '% this week</div>';
    }
