...
function historyColumnExists($conn, $table, $column) {
...
$hasReservationMode = historyColumnExists($conn, 'orders', 'ReservationMode');
$hasSeatCount = historyColumnExists($conn, 'orders', 'SeatCount');
$hasOrderDate = historyColumnExists($conn, 'orders', 'OrderDate');
$hasCreatedAt = historyColumnExists($conn, 'orders', 'CreatedAt');
...
$chatCheck = mysqli_query($conn, "SHOW TABLES LIKE 'chat_messages'");
if ($chatCheck && mysqli_num_rows($chatCheck) > 0) {
    $chatTableExists = true;
}

// Fetch all orders for this user
$sql = "SELECT o.OrderID, o.OrderType, o.PickupTime, o.TotalAmount, o.Status,
               " . ($hasReservationMode ? "o.ReservationMode" : "NULL AS ReservationMode") . ",
               " . ($hasSeatCount ? "o.SeatCount" : "NULL AS SeatCount") . ",
               $dateCol AS CreatedDate,
               o.CancelReason,
...
while ($row = mysqli_fetch_assoc($result)) {
    // Count chat messages for this order
    $row['ChatCount'] = 0;
    if ($chatTableExists) {
        $chatSql = "SELECT COUNT(*) AS cnt FROM chat_messages WHERE OrderID = ?";
        $chatStmt = mysqli_prepare($conn, $chatSql);
        mysqli_stmt_bind_param($chatStmt, "i", $row['OrderID']);
        mysqli_stmt_execute($chatStmt);
        $chatRes = mysqli_stmt_get_result($chatStmt);
        $chatRow = mysqli_fetch_assoc($chatRes);
        $row['ChatCount'] = (int)$chatRow['cnt'];
    }
...
$activeOrder = null;
$activeSql = "SELECT OrderID, Status FROM orders WHERE UserID = ? AND Status NOT IN ('Finished', 'Completed', 'Cancelled') ORDER BY OrderID DESC LIMIT 1";
$activeStmt = mysqli_prepare($conn, $activeSql);
mysqli_stmt_bind_param($activeStmt, "i", $userId);
mysqli_stmt_execute($activeStmt);
$activeRes = mysqli_stmt_get_result($activeStmt);
if ($activeRes) {
    $activeOrder = mysqli_fetch_assoc($activeRes);
}
?>
<!DOCTYPE html>
<html lang="en">
