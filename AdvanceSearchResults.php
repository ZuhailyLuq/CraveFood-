...
function menuColumnExists($conn, $column) {
    $columnEsc = mysqli_real_escape_string($conn, $column);
    $res = mysqli_query($conn, "SHOW COLUMNS FROM `MENU_FOOD` LIKE '$columnEsc'");
    return $res && mysqli_num_rows($res) > 0;
}

$hasRating = menuColumnExists($conn, 'Rating');
...
if ($keyword !== '') {
    ...
    $conditions[] = "(mf.FoodName LIKE '%$kwEsc%' OR mf.Description LIKE '%$kwEsc%' OR v.ShopName LIKE '%$kwEsc%')";
}
if ($dietary !== '') {
    $dietEsc = mysqli_real_escape_string($conn, $dietary);
    $conditions[] = "mf.DietaryTag = '$dietEsc'";
}
if ($category !== '') {
    $catEsc = mysqli_real_escape_string($conn, $category);
    $conditions[] = "mf.Category = '$catEsc'";
}
if ($minPrice !== '' && is_numeric($minPrice)) {
    $conditions[] = "mf.Price >= " . (float)$minPrice;
}
if ($maxPrice !== '' && is_numeric($maxPrice)) {
    $conditions[] = "mf.Price <= " . (float)$maxPrice;
}
...
if ($sessionUserId > 0) {
    ...
    mysqli_stmt_bind_param($activeStmt, "i", $sessionUserId);
