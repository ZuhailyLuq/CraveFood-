...
if ($isSearch) {
    if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
        ...
        $sql .= " AND (mf.FoodName LIKE '%$searchQuery%' OR mf.Description LIKE '%$searchQuery%' OR v.ShopName LIKE '%$searchQuery%')";
    }
    // Support both legacy single 'dietary' and new multi-select 'dietary_cb[]'
    if (!empty($_GET['dietary_cb']) && is_array($_GET['dietary_cb'])) {
        $safeTags = [];
        foreach ($_GET['dietary_cb'] as $tag) {
            $safeTags[] = "'" . mysqli_real_escape_string($conn, $tag) . "'";
        }
        $dietaryFilters = $_GET['dietary_cb'];
        $sql .= " AND mf.DietaryTag IN (" . implode(',', $safeTags) . ")";
    } elseif (isset($_GET['dietary']) && !empty($_GET['dietary'])) {
        $dietaryFilter = mysqli_real_escape_string($conn, $_GET['dietary']);
        $dietaryFilters = [$dietaryFilter];
        $sql .= " AND mf.DietaryTag = '$dietaryFilter'";
    }
    if (isset($_GET['order_type'])) {
        $orderType = mysqli_real_escape_string($conn, $_GET['order_type']);
    }
    if (isset($_GET['pickup_time'])) {
        $pickupTime = mysqli_real_escape_string($conn, $_GET['pickup_time']);
    }
}

// Pull user dietary preference if column exists
if ($userId > 0 && columnExists($conn, 'user', 'DietaryPreference')) {
    $prefSql = "SELECT DietaryPreference FROM `user` WHERE UserID = ?";
...
if (!$isSearch) {
    $recommendProfile = buildRecommendationProfile($conn, $userId, $userDietPreference);
    $recommendNote = getRecommendationNote($recommendProfile);
    $recommendedFoods = fetchRecommendedFoods($conn, $recommendProfile);
}
